...
with Diagram("Initial Diagram", direction="RL", show=False):
    with Cluster("Cloud Hosted"):
        ...
        with Cluster("User Interface"):
            ui = Docker("Streamlit")
        database = Database("Any Database")
        bucket = S3("Assignment 4 Bucket")

    RestApI = API("Chat GPT API")

    database >> Edge(label="Get questionare",color="firebrick", style="dashed") >> ui
    airflow1 >> Edge(label="Write Questionare",color="firebrick", style="dashed") >> database
